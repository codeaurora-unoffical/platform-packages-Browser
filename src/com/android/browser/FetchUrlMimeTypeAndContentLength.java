 ...
 *
 */
class FetchUrlMimeTypeAndContentLength extends Thread {

    private final static String LOGTAG = "FetchUrlMimeTypeAndContentLength";
    ...
    public FetchUrlMimeTypeAndContentLength(Activity activity, String url, String userAgent,String referer, boolean privateBrowsing, String filename) { 
        mActivity = activity;
    ...
    public void run() {
        ...
        String mCookies = CookieManager.getInstance().getCookie(mUri, mPrivateBrowsing);
        ...
        if (mimeType != null) {
            ...
            if (mimeType.equalsIgnoreCase("text/plain") ||
                ...
                String newMimeType = MimeTypeMap.getSingleton().getMimeTypeFromExtension(MimeTypeMap.getFileExtensionFromUrl(mUri));
                if (newMimeType != null) {
        ...
        DownloadHandler.startDownloadSettings(mActivity, mUri, mUserAgent, contentDisposition , mimeType, mReferer, mPrivateBrowsing, mContentLength, filename);
    }

    /** 
      when we can not parse MineType and Filename from the header of
      http body ,Call the fallowing functions for this matter
      getFileExtensionFromUrlEx(String url) : get the file Extension
      from Url
      guessFileNameEx() : get the file name from url
      Note:
      this modified for download http://www.baidu.com girl picture
      error extension and error filename
    */
    private String getFileExtensionFromUrlEx (String url) {
        Log.e("FetchUrlMimeType","--------can not get mimetype from http header, the URL is ---------->"+url);
        if (!TextUtils.isEmpty(url)) {
            ...
            Log.e(LOGTAG,"--------can not get mimetype from http header, the temp filename is----------"+filename);
